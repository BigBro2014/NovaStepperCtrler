...
public:
  ...
  // min_velocity = clk_rate / 2 / counts_per_rev * PI * wheel_radius / 65536
  void init(uint8_t left_clk_pin, uint8_t right_clk_pin, uint8_t left_dir_pin, uint8_t right_dir_pin, float wheel_radius, uint16_t counts_per_rev, CLK_RATE clk_rate=CLK_15625HZ);
...
private:
  ...
  int8_t _curr_dir[2];            // current dir of stepper motors
